...
if fid < 1
    fprintf(fid_log, '%s\n%s\n', ' ...exiting...', ...
        ['*** SXPParse.m Error : requested parameter file ' DataFileName ' not found ! ']);
end;

if fid > 0
    fprintf(fid_log, '\n%s\n%s\n', 'reading parameter data from file ', [ DataFileName ' ...']);

    % --- find out matrix order ---
    [NoOfPorts, ~] = strtok(fliplr(DataFileName), '.');    
    NoOfPorts = strtok(fliplr(NoOfPorts), 'p');
    NoOfPorts = str2double(NoOfPorts(2:end));
    ...
    while ~flagNoiseStarted && flagGotOptions
        ...
        if ~isempty(word) % && ~strcmp(word(1,1),'!')
            ...
            thisFreqTerms = thisFreqTerms + size(data_row,2);
				...
				% disp(['tFT=' num2str(thisFreqTerms)]);
                % disp(['A=' num2str(thisFreqTerms+1)]);
                % disp(['Datarow=' num2str(data_row)]);
				% disp(['Datarows=' num2str(size(data_row))]);
				% disp(['Datarows 2=' num2str(size(data_row,2))]);
                % disp(['B=' num2str(thisFreqTerms+size(data_row,2))]);
                return
            end
        end
