        ...
        for j=1:length(M11_W)
            for k=1:length(M11_S)
                for l=1:length(NT)
                    ...
                    [tmpDataFiles replot] = t40ind_wh(tmpScriptFile, fid, OD(i), M11_W(j), M11_S(k), M9_W(j), M9_S(k), NT(l));    
