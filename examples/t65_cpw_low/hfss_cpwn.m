function serial = hfss_cpwn(fid, M9_W, M9_S, M9_L,GND_H,GND_F, serial)
        ...
        serial = hfss_gndvd(fid, '-$M9_S-0.5*$M9_W', strcat('-',M9_L,'/2') ,'$M3_Z', '2*$M9_S+$M9_W', '$M9_L',GND_H, GND_F, '$M3_T', serial, 'ME0d022'); % gnd mesh        
        serial = hfss_lead(fid, strcat('-', M9_S,'-1.5*',M9_W),strcat('-',M9_L,'/2') , '$M3_Z' , M9_W, M9_L, '$M3_T','$VIA_W','$M9_Z-$M3_Z-$M3_T', serial,'ME0d022');
        serial = hfss_lead(fid, strcat(M9_S,'+0.5*',M9_W),strcat('-',M9_L,'/2') , '$M3_Z' , M9_W, M9_L, '$M3_T','$VIA_W','$M9_Z-$M3_Z-$M3_T', serial,'ME0d022');
        ...
        hfssCreateVar(fid, 'PORT_H', '$M9_Z-$M3_Z-$M3_T');
        % port        
        posi = {strcat('-', M9_W,'/2'), strcat('-', M9_L,'/2'),'$M3_Z+$M3_T'};
        poso = {strcat('-', M9_W,'/2'), strcat(M9_L,'/2'),'$M3_Z+$M3_T'};
