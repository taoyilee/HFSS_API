...
try
	...
	time = 0;
	outdir = 'G:\HFSS_SIMULATIONS\t65_xfm_may1\';
    if(~exist(outdir,'dir'))
        mkdir(outdir);
    end
    var_fp =  [num2str(OD(1)) ',' num2str(M9_W(1)) ',' num2str(M9_S(1))  ];
    if(~exist([outdir '/empty.hfss'],'file'))
        fid = fopen(tmpScriptFile, 'w+');    
        param = hfssProcess(fid,'../T65/t65me6X1Z1U.csv','../T65/IMD_6X1Z1UIMD_8g_IMD_9c.csv' ,'6X1Z1U', 'var_T65_xfm', var_fp,0,outdir);
        fclose(fid);
    ...
    for i=1:length(OD)
        for j=1:length(M9_W)
            for k=1:length(M9_S)
                    ...
                    [tmpDataFiles replot] = t65_xfm_wh(tmpScriptFile, fid, OD(i), M9_W(j), M9_S(k), outdir);    
                    ...
                    try
                        ...
                        disp(['*** Iteration failed-> OD' num2str(OD(i)) 'W' num2str(M9_W(j)) 'S' num2str(M9_S(k))]);
                        disp('Continuing ...');
                        time = toc; % stop stopwatch
                        disp(['Time Elapsed : ' num2str(time/60) ' min.']); % report simulation time for current iter.                        
                        time = (time*iter + time)/(iter+1); % compute average simulation time
