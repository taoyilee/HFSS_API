...
try
    ...
    iconized = true ;
    tmpScriptFile = 'G:\temp\t65_xfm_m8.vbs';
	...
	time = 0;
	outdir = 'G:\HFSS_SIMULATIONS\t65_xfm_M8\';
    if(~exist(outdir,'dir'))
        mkdir(outdir);
    end
    ...
    if(~exist([outdir '/empty.hfss'],'file'))
        ...
        param = hfssProcess(fid,'../T65/t65me6X1Z1U.csv','../T65/imd_simplified.csv' ,'6X1Z1U', 'var_T65_xfm', var_fp,0,outdir,'empty.hfss');
        fclose(fid);
        hfssExecuteScript(hfssExePath, tmpScriptFile,iconized,runAndExit,nowait);
    end
    
    fid = fopen(tmpScriptFile, 'w+');  
    t65_xfm_nom(tmpScriptFile, fid, OD(1), M9_WI(1),M9_WO(1), M9_S(1), outdir) 
    fclose(fid);    
    hfssExecuteScript(hfssExePath, tmpScriptFile,iconized,runAndExit,nowait);
        ...
        for j=1:length(M9_WI)
            for k=1:length(M9_WO)
                for l=1:length(M9_S)
                    ...
                    [tmpDataFiles replot] = t65_xfm_wh(tmpScriptFile, fid, OD(i), M9_WI(j),M9_WO(k), M9_S(l), outdir);    
