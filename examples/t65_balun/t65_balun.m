...
try
    ...
    iconized = true ;
    tmpScriptFile = 'G:\temp\t65_balun.vbs';
    ...
    M9_WO = 2; 
    M9_S = 2;
    % dlmwrite('G:\HFSS_SIMULATIONS\t65_balun\summary.csv', ...
	%	'OD, W, S, NT, L(5.8G),Q(5.8G),L(10.6G),Q(10.6G),L(24G),Q(24G)','delimiter','');
	iter = 1;
	total = length(OD)*length(M9_WI)*length(M9_WO)*length(M9_S);
	time = 0;
	outdir = 'G:\HFSS_SIMULATIONS\t65_balun\';
    if(~exist(outdir,'dir'))
        mkdir(outdir);
    end
    var_fp =  [num2str(OD(1)) ',' num2str(M9_WI(1)) ',' num2str(M9_WO(1)) ',' num2str(M9_S(1))  ];
    if(~exist([outdir '/empty.hfss'],'file'))
        fid = fopen(tmpScriptFile, 'w+');    
        param = hfssProcess(fid,'../T65/t65me6X1Z1U.csv','../T65/imd_simplified.csv' ,'6X1Z1U', 'var_T65_balun', var_fp,0,outdir,'empty.hfss');
        fclose(fid);
        hfssExecuteScript(hfssExePath, tmpScriptFile,iconized,runAndExit,nowait);
    end
    
    fid = fopen(tmpScriptFile, 'w+');  
    t65_balun_nom(tmpScriptFile, fid, OD(1), M9_WI(1),M9_WO(1), M9_S(1), outdir) 
    fclose(fid);    
    hfssExecuteScript(hfssExePath, tmpScriptFile,iconized,runAndExit,nowait);
        ...
        for j=1:length(M9_WI)
            for k=1:length(M9_WO)
                for l=1:length(M9_S)
                    ...
                    [tmpDataFiles replot] = t65_balun_wh(tmpScriptFile, fid, OD(i), M9_WI(j),M9_WO(k), M9_S(l), outdir);    
