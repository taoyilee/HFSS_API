...
try
    ...
    serial = 0;
	outdir = 'G:\HFSS_SIMULATIONS\t65_cpw_diff_GS5\';
    if(~exist(outdir,'dir'))
        mkdir(outdir);
    end
    M9_W = 2;
    ...
    dlmwrite([outdir 'summary.csv'], ...
		'W, GS, SS, L, dbS11(5.8G),dbS21(5.8G),dbS11(10.6G),dbS21(10.6G),dbS11(24G),dbS21(24G)','delimiter','');
	iter = 1;
	total = length(M9_W)*length(M9_GS)*length(M9_SS)*length(M9_L);
	time = 0;
    var_fp =  [num2str(M9_W(1)) ',' num2str(M9_GS(1)) ','  num2str(M9_SS(1)) ',' num2str(M9_L(1))  ',' num2str(GND_H(1)) ',' num2str(GND_F(1))];
    ...
    for i=1:length(M9_W)
        for j=1:length(M9_GS)
            for k=1:length(M9_SS)
                for l=1:length(M9_L)                
                        tic; % start stopwatch
                        ...
                        [tmpDataFiles replot] = t65cpw_diff_wh(tmpScriptFile, fid, M9_W(i), M9_GS(j), M9_SS(k), M9_L(l), GND_H(1), GND_F(1),outdir);    
                        % Close the HFSS Script File.
                        fclose(fid);
                        try
                            disp('Script received, simulation start ...');                        
                            hfssExecuteScript(hfssExePath, tmpScriptFile,iconized,runAndExit,nowait);                        
                            % plotS11S21_diff(tmpDataFiles,M9_W(i), M9_GS(j), M9_SS(k), M9_L(k), GND_H(1), GND_F(1), replot);
                            ...
                            disp(['*** Iteration failed-> W' num2str(M9_W(i)) 'GS' num2str(M9_GS(j)) 'SS' num2str(M9_SS(k))  'L' num2str(M9_L(l))]);
