function serial = hfss_cpwn(fid, M9_W, M9_S, M9_L,GND_H,GND_F, serial)
        ...
        serial = hfss_gndvd(fid, '-$M9_S-0.5*$M9_W', strcat('-',M9_L,'/2') ,'$M6_Z', '2*$M9_S+$M9_W', '$M9_L',GND_H, GND_F, '$M6_T', serial, 'ME0d022'); % gnd mesh        
        serial = hfss_lead(fid, strcat('-', M9_S,'-1.5*',M9_W),strcat('-',M9_L,'/2') , '$M6_Z' , M9_W, M9_L, '$M6_T','$VIA_W','$M9_Z-$M6_Z-$M6_T', serial,'ME0d022');
        serial = hfss_lead(fid, strcat(M9_S,'+0.5*',M9_W),strcat('-',M9_L,'/2') , '$M6_Z' , M9_W, M9_L, '$M6_T','$VIA_W','$M9_Z-$M6_Z-$M6_T', serial,'ME0d022');
        ...
        % port        
        posi = {strcat('-', M9_W,'/2'), strcat('-', M9_L,'/2'),'$M6_Z+$M6_T'};
        poso = {strcat('-', M9_W,'/2'), strcat(M9_L,'/2'),'$M6_Z+$M6_T'};
