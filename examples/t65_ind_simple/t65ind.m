...
try
    ...
    init_hfssapi;		
    runAndExit = true ;
    tmpScriptFile = 'G:\temp\t65_ind_simple.vbs';
    ...
    % NT =1;
    NT = 2;
    outdir = 'G:\HFSS_SIMULATIONS\t65_ind_fast_6\';
    if(~exist(outdir,'dir'))
        mkdir(outdir);
    end
    ...
    if(~exist([outdir '/empty.hfss'],'file'))
        ...
        param = hfssProcess(fid,'../T65/t65me6X1Z1U.csv','../T65/imd_simplified.csv' ,'6X1Z1U', 'var_T65_ind', var_fp,0,outdir,'empty.hfss');
        % param = hfssProcess(fid,'../T65/t65me6X1Z1U.csv','../T65/imd_simplifiedB.csv' ,'6X1Z1U', 'var_T65_ind', var_fp,0,outdir,'empty.hfss');
    ...
    fid = fopen(tmpScriptFile, 'w+');  
    t65ind_nom(tmpScriptFile, fid, OD(1), 1, M9_W(1), M9_S(1), outdir)
    fclose(fid);    
    hfssExecuteScript(hfssExePath, tmpScriptFile,iconized,runAndExit,nowait,'');
        ...
        for j=1:length(M9_W)
            for k=1:length(M9_S)
                for l=1:length(NT)
                        ...
                        [tmpDataFiles replot] = t65ind_wh(tmpScriptFile, fid, OD(i), M9_W(j), M9_S(k), NT(l),outdir);    
